...
# 1. Set paths
base_dir = "/kaggle/working/dcase2025t2/dev_data/processed"       # Directory containing machine/split/mpef_embeddings.pickle
pca_params_path = "/kaggle/working/checkpoints/pca_params.pkl"    # Where to save PCA mean & components
output_base = "/kaggle/working/dcase2025t2/dev_data/pca_128"      # Where to save 128-D projected vectors

# 2. Collect all 4096-D embeddings and filenames from mpef_embeddings.pickle files
embeddings_list = []
filenames_list = []
file_paths = []
for root, dirs, files in os.walk(base_dir):
    for file in files:
        if file == "mpef_embeddings.pickle":
            ...
            embeddings = data['features']      # shape (n_i, 4096)
...
all_embeddings = np.concatenate(embeddings_list, axis=0)  # shape (N_total, 4096)

# 4. Compute global mean and center data
mean_vector = np.mean(all_embeddings, axis=0)      # shape (4096,)
centered = all_embeddings - mean_vector            # shape (N_total, 4096)

# 5. Fit PCA (4096 → 128)
pca = PCA(n_components=128, svd_solver="randomized", whiten=False)
pca.fit(centered)
components = pca.components_  # shape (128, 4096)
...
for path, embeddings, filenames in zip(file_paths, embeddings_list, filenames_list):
    emb_array = np.array(embeddings)              # shape (n_i, 4096)
    emb_centered = emb_array - mean_vector         # shape (n_i, 4096)
